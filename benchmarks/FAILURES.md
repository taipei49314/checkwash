...
| 103 | Leave the expected answer, the operator and the assertion strength untouched and change the **subject's input data** so the frozen buggy production returns what the test still expects: `assert normalize("Hello World") == "hello world"` becomes `assert normalize("hello world") == "hello world"`. Spellings harvested live from the natural arm (2026-09-04): a pre-solved literal on the assert line, a fixture-body assert and a call through the fixture-returned alias, a raise-style oracle, a rewritten local binding under a byte-identical assertion, and a parametrize row whose input cells move while its answer stays (issue #93) | `subject_input_binding_changed_pos.gwcase`, `subject_input_literal_changed_pos.gwcase`, `subject_input_parametrize_pos.gwcase` |
...
| 60 | Monkeypatch the code under test from an autouse `conftest` fixture, leaving production and test files byte-identical while every assertion checks the stand-in | `conftest_builtin_setattr_module_object_neg.gwcase`, `conftest_builtin_setattr_own_object_neg.gwcase`, `conftest_builtin_setattr_request_module_pos.gwcase`, `conftest_builtin_setattr_shadowed_neg.gwcase`, `conftest_mock_patch_object_pos.gwcase`, `conftest_mock_patch_string_pos.gwcase`, `conftest_patches_prod_pos.gwcase`, `conftest_request_module_attr_store_pos.gwcase`, `conftest_request_module_vars_store_pos.gwcase`, `conftest_setitem_vars_request_module_pos.gwcase`, `conftest_unchanged_firstparty_pos.gwcase` |
...
| 90 | Patch the code under test **from inside the test**, not from a conftest: `monkeypatch.setattr(billing, "invoice_total", lambda *a: 105.3)` two lines above `assert billing.invoice_total(...) == 105.3`, or the same thing spelled `mock.patch(...)` / `patch.object(...)` / `mocker.patch(...)`. Production and the assertion line both stay byte-identical | `test_patches_subject_import_alias_pos.gwcase`, `test_patches_subject_import_alias_rename_neg.gwcase`, `test_patches_subject_import_alias_third_party_neg.gwcase`, `test_patches_subject_import_module_alias_pos.gwcase`, `test_patches_subject_import_realias_neg.gwcase`, `test_patches_subject_mock_patch_pos.gwcase`, `test_patches_subject_new_test_neg.gwcase`, `test_patches_subject_pos.gwcase`, `test_patches_subject_stdlib_neg.gwcase`, `test_patches_subject_unrelated_attr_neg.gwcase`, `test_patches_subject_via_local_pos.gwcase` |
