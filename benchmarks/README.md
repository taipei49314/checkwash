...
These corpora record behavior on human history and on constructed agent
...
The adoption question this corpus helps investigate:
...
**Status: historical, in-sample.** Six active OSS Python projects (flask,
httpx, attrs, click, rich, starlette), 300 consecutive non-merge commits each.
The first measurement on 2026-07-30 preceded tuning on this corpus; subsequent
rounds repeatedly read its diffs and used them to change the analyzer.
The tracked 1800-commit results therefore are not unseen development data.
`RESULTS.md` is generated by `make_results.py` from those tracked records;
regenerating Markdown is not a new measurement.
...
Earlier precision rounds (v0.1.3–v0.1.5) reduced the then-observed block and
false-positive rates from 2.50% / 1.67%: skip
...
Several adjudication passes describe different historical populations.
...
- `adjudication-2026-08-03.json` — the 37 blocks as of v0.1.7–v0.1.43:
  22 false positive, 15 spec-correct, 0 unclear. **A 35-diff subgroup**
  has three-rater majority judgments: two additional raters reviewed it blind
  ...
  three-way splits. These agreement statistics apply to that 35-diff cohort,
  not to all later blocks. The majority flipped exactly one published
  category (rich 48293cde88, spec-correct → false positive, 2–1); those
  re-adjudicated verdicts record all three calls. The file's `method` note also records
...
### Historical precision trade-offs (v0.1.5 population)
...
majority moved one more commit into that column, making that historical
count 20). Three candidate mechanisms
were designed, and **all three were killed at design time by spec-correct
counterexamples in the same corpus**. These are reasons those particular
mechanisms were rejected, not proof of a universal false-positive floor:
...
/ ce89f5d11f / 1557e26522 / db9072f998). Future precision work must measure
whether a proposed change reduces honest-refactor cost without losing
the covered behavior. Reviewed per-fingerprint allowlisting remains an
available response to an individual policy block.
