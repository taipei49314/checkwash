...
> Status: **pre-release.** 15 detectors, 223 tests, zero runtime dependencies.
...
- **Human-commit block rate — 36 / 1800 = 2.00%.** Six active OSS projects
  ...
  with nothing visible replacing it, 0 unclear. Two precision rounds brought
  this down from 2.50% / 1.67%: skip conditions are now *read* (constants
  ...
  assertions, and feature removals and dependency bumps explain the removal
  of their tests — every legitimate policy block still blocks, the decoy
  corpus still blocks 12/12, and one over-eager version of the
  feature-removal credit was caught clearing two correct blocks and
  tightened before it shipped; each trade is reported as measured.
...
- uses: taipei49314/greenwash/action@v0.1.4
```
...
```bash
pipx install git+https://github.com/taipei49314/greenwash@v0.1.4
# or: uv tool install git+https://github.com/taipei49314/greenwash@v0.1.4
