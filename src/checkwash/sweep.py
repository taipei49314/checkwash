    ...
    for sha in commits:
        ...
        try:
            ir, findings, verdict = analyze(
                ...
                head_searcher=lambda needles, _sha=sha: HeadSearchResult(
                    tuple(grep_head_paths(repo, _sha, needles)), complete=True
                ),
                head_path_lister=lambda _sha=sha: list_tree_paths(repo, _sha),
                head_batch_reader=lambda paths, _sha=sha: read_tree_files(repo, _sha, paths),
            )
