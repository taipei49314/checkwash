...
from checkwash.frontends.python.oracle_collections import fresh_interleave, fresh_unique_merge
...
def _expression(node, names):
    ...
    if isinstance(node, ast.Call):
        if (isinstance(node.func, ast.Attribute)
                and node.func.attr in {'replace', 'strip', 'lstrip', 'rstrip', 'split', 'rsplit', 'join', 'lower', 'upper', 'casefold', 'startswith'}):
