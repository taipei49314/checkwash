...
from checkwash.allowlist import MAX_EXPIRY_DAYS
...
def render(
    ...
    if not visible and not suppressed:
        ...
        if blocking:
            head = (
                f"checkwash: {n_at_or_above} finding(s) at or above {fail_on} "
                f"{sym['dash']} blocking"
            )
            lines.append(_c("31", sym["block"] + " " + head, color))
        else:
            head = f"checkwash: {len(visible)} finding(s), none at or above {fail_on}"
    ...
    for f in visible:
        ...
        if f.severity in ("high", "critical"):
            ...
            lines.append(f'    checkwash allow "{f.fingerprint}" --reason "..."')
