"""`greenwash demo` — replay real tampering cases offline, in seconds.
...
`greenwash check` runs — no network, no API key, no LLM — and shows the crime,
...
def _load_cases() -> list[tuple[str, str]]:
    pkg = resources.files("greenwash").joinpath("demo_cases")
    out = []
...
def run(stream=None) -> int:
    ...
    cases = _load_cases()
    if not cases:
        w("greenwash demo: no cases packaged\n")
        return 2

    w("\ngreenwash demo — replaying real tampering cases, fully offline\n")
    w("=" * 64 + "\n\n")
