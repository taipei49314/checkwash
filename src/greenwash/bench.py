"""`greenwash bench` — reproduce the published numbers from this checkout.
...
def collect(
    ...
    if checkout is None:
        report.errors.append(
            ...
            "and run `greenwash bench` from that tree (or pass --repo pointing at it)"
        )
        return report
...
def render(report: BenchReport, stream=None, local_only: bool = False) -> int:
    ...
    w("greenwash bench — reproduce the published numbers\n")
    w("=" * 56 + "\n\n")
    ...
    if report.checkout is None:
        for err in report.errors:
            w(f"FAIL  {err}\n")
        w("\nsummary: not a greenwash checkout; nothing was reproduced.\n")
        return 2
    ...
    for clone in report.clones:
        ...
        if mark == "MISS" and clone.remote and clone.newest:
            ...
            w(f"       greenwash sweep HEAD --limit 300 --repo {clone.name}\n")
    ...
    if report.missing_clones:
        ...
        w("Then re-run `greenwash bench` or, to actually sweep:\n")
        w("  greenwash bench --corpus DIR --run-sweep\n")
