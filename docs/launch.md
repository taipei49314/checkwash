...
Integrations — GitHub Action (the doctor-verified prior stable pin `ec58e9f…`, v0.1.41), pre-commit (`rev: v0.1.41`), and a Claude Code stop-hook that checks the working tree the moment the agent finishes, before the diff is even a commit. The Action has an explicit one-release trust lag because a release cannot embed its own commit SHA; the local integrations use the advertised install tag. The advertised tag's `src/`, `action/` and pre-commit hook are diffed against the working tree by a test, because the README once pinned a tag two fixes behind main.
