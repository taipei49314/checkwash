"""Preregistered historical native/profile compatibility audit v5.1; hosted only.
...
HARNESS_AMENDMENT = {
    "revision": "5.1",
    "supersedes_measurements_from_run": "34364672728",
    "prior_runner_sha256_lf": "9a92c97745f4ed7ac2530c3fa618f8b41c8dc738546b5d85f66a2d37f26114f9",
    "prior_results_policy": "retain original run and all its errors/divergences unchanged; amended run is a new observation",
    "changes": ["parse every Ruff rule token, including modern name/CODE pairs and pre-0.4 Rust debug names, against the same installed version's native rule catalog; reject unmatched/nonempty unparsed tokens", "observe mypy strict option Boolean deltas via process_options on both strict off/on instead of importing define_options absent in older versions; strict flags already equal to their strict value by default may not appear, so this is not a complete historical strict-assignment registry", "retain raw Ruff defaults settings and compare the native exclude list against unchanged profile; retain build exclusion discrepancy", "retain partial stdout/stderr on subprocess timeout"],
    "unchanged": ["25-version selection", "625 facet case identities and expected frozen profile values", "all synthetic config/program content already exercised in v5 except additional strict-delta observation pair", "product source and profile data", "human labels", "natural execution remains prohibited"],
}
GAPS = {
...
def run_command(argv, cwd=None, timeout=180, env=None):
    try:
        ...
        return {"argv": [str(x) for x in argv], "returncode": 124, "stdout": readable(exc.stdout), "stderr": readable(exc.stderr), "timeout_seconds": timeout, "status": "TIMED_OUT_UNMEASURED"}
...
class Facets:
    ...
    def enabled(self, root, *args):
        ...
        if "--isolated" in args:
            match_exclude = re.search(r"file_resolver\.exclude\s*=\s*\[(.*?)\]", output, re.S)
            native_excludes = re.findall(r'"([^"\n]+)"', match_exclude.group(1)) if match_exclude else None
            dump(self.args.output / "native-default-settings-observation.json", {"raw_settings": output, "parsed_native_excludes": native_excludes, "parser_status": "PARSED" if native_excludes is not None else "OLD_DEBUG_FORMAT_RETAINED_UNPARSED", "frozen_profile_excludes": self.profile["default_excludes"], "profile_only": sorted(set(self.profile["default_excludes"]) - set(native_excludes)) if native_excludes is not None else None, "native_only": sorted(set(native_excludes) - set(self.profile["default_excludes"])) if native_excludes is not None else None})
        return parse_ruff_enabled(output, self.native_catalog)


def parse_ruff_enabled(output, catalog):
        match = re.search(r"linter\.rules\.enabled\s*=\s*\[(.*?)\]", output, re.S)
        if not match:
            match = re.search(r"rules:\s*RuleTable\s*\{\s*enabled:\s*\{(.*?)\}", output, re.S)
        if not match:
        ...
        if not isinstance(catalog, list):
            raise ValueError("Exact-version native rule catalog required to parse enabled rules")
        name_map = {}
        for rule in catalog:
            key = re.sub(r"[^a-z0-9]", "", rule["name"].lower())
            name_map.setdefault(key, set()).add(rule.get("code"))
        codes, unmatched = [], []
        for token in tokens:
            coded = re.fullmatch(r"([a-z0-9-]+)\s+\(([A-Z]+\d+)\)", token)
            if coded:
                key = re.sub(r"[^a-z0-9]", "", coded.group(1))
                code = coded.group(2)
                if name_map.get(key) != {code}:
                    unmatched.append(token)
                else:
                    codes.append(code)
                continue
            # Ruff >=0.16 also displays uncoded rule names; these are not part
            # of the frozen code-set dimension, but must be catalog-authenticated.
            key = re.sub(r"[^a-z0-9]", "", token.lower())
            choices = name_map.get(key, set())
            if len(choices) != 1 or not re.fullmatch(r"[A-Za-z0-9-]+", token):
                unmatched.append(token)
            else:
                code = next(iter(choices))
                if code is not None:
                    codes.append(code)
        if unmatched:
            raise ValueError("Unmapped native enabled-rule tokens: " + json.dumps(unmatched))
...
def mypy_facets(f):
    ...
    def strict_map():
        ...
        for enabled in (False, True):
            root = f.fixture("strict-map-" + str(enabled), {"mypy.ini": "[mypy]\nstrict=" + str(enabled).lower() + "\n", "sample.py": "def f(x):\n    return x\n"})
            _, options = process_options(["--config-file", str(root / "mypy.ini"), str(root / "sample.py")])
            pair.append({name: getattr(options, name) for name in dir(options) if not name.startswith("_") and isinstance(getattr(options, name), bool)})
        native = {name: value for name, value in pair[1].items() if pair[0].get(name) != value}
        dump(f.args.output / "native-strict-expansion-observation.json", {"method": "all observable public Boolean option differences with only strict toggled false to true", "limitation": "a historical strict-assignment already equal to its default may not appear in deltas; this observation is not the full strict-assignment registry", "strict_false_boolean_options": pair[0], "strict_true_boolean_options": pair[1], "strict_boolean_deltas": native, "frozen_profile_strict_expansion": f.profile["strict_expansion"]})
        return native
    f.check("strict-expansion-map", "mypy.global_flags", f.profile["strict_expansion"], strict_map)
