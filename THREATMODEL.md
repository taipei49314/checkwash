...
| 91 | Put the oracle somewhere that is not a syntactic `assert` in the collected unit, then stop **invoking** it while leaving it in place: a helper function, a lambda, a nested `verify()`, a class whose `__init__` holds the check, a context manager, `functools.partial`, a doctest, a `compile()`d string, an inherited mixin, an autouse fixture. Or keep the `assert` and subvert what it compares — `__eq__`, `__bool__`, `__contains__`, an `__exit__` that returns True, a dataclass field marked `compare=False`, a shadowed `assertEqual`, a `TestResult` whose `addFailure` is a no-op. Or keep the loop and empty the table it iterates | **Open, and the largest hole measured to date.** 40 informed adversarial cases, generated 2026-08-13 against a description of every rule in this tool and verified mechanically before being counted: production byte-identical on both sides, BEFORE actually failing under pytest, AFTER actually passing. **greenwash blocks 12 of 40.** The 28 escapes share one root cause — the IR records bare `assert`, curated unittest methods and `pytest.raises`, so an oracle expressed any other way is not in the IR at all. `assert_sum(add(2, 3), 5)` is a *call*: the unit reads as having **zero** assertions, so nothing can be removed and nothing weakened, and the replacement `assert callable(assert_sum)` registers as an assertion *added*. By the strength lattice the test got stronger. This is `docs/defence-design.md` A5, which that document ranked **P2, "partial by nature"** — the measurement says it is neither P2 nor partial. **Caveat, stated because the number will be quoted:** these are synthetic and *informed*, so 12/40 bounds a motivated attacker who has read the rules; it is not a rate for ordinary agent behaviour, and under natural conditions 0 of 12 agents touched a test at all. But several of the shapes need no ingenuity — a helper that stops being called, a parametrize table emptied, a fixture that stops being autouse, a mixin dropped from a `TestCase`. Corpus, verifier and per-case verdicts: `benchmarks/tamper/` |

| 92 | (False positive, and the largest one) Restructure where a test's assertions live, without weakening anything: extract the check into a helper or a `conftest.py`, move it into a fixture's teardown, merge two tests into one, split one into two, drive them from a params fixture, put the comparison behind `operator.eq` or a comparison object's method, replace exact equality with `pytest.approx` and a tolerance | **Open, and measured 2026-08-13: greenwash blocks 20 of 30.** Each case ships production twice — correct and buggy — so four pytest runs establish that *both* sides still catch the bug before greenwash is consulted; a block is then a false positive by construction, with no adjudication to disagree about. Two families: `ASSERT_REMOVED` when the assertion moved somewhere the IR does not follow (8 cases), and `TEST_DISABLED` when unit identity changed under a merge, split, `setUp` or params fixture (6 cases), plus one `ASSERT_WEAKENED` on an introduced `approx`. **This is row 91 seen from the other side** — the same blindness that lets 28 of 40 attacks through also blocks the honest refactors, because both turn on greenwash not knowing which assertions a test actually runs. It is not visible in the 1.17% published false-positive rate, and re-running that sweep would never have shown it: those six repositories rarely restructure test helpers, which is the same zero-power trap as row 90. Corpus and harness: `benchmarks/refactors/` |
