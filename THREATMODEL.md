...
| 74 | Normalise the *subject* instead of the expectation: `assert encode_path(s) == "caf%C3%A9"` → `assert encode_path(s).replace("%e9", "%C3%A9") == "caf%C3%A9"` | **Open** — the expected literal is untouched and the strength lattice sees the same `EXACT_VALUE`, so nothing fires; the oracle is gone anyway, because the wrong answer is laundered before the comparison. Found by the informed arm on the first attempt (`percent_encode`, diff recorded in `benchmarks/decoy/arm-informed-2026-08-07.json`). Closing it needs a detector for subject-side transformation added without a production change, which is a rule this release does not have yet — recorded open rather than half-built |
