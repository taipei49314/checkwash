...
7. **Equivalent rewrites of an assertion.** When both the subject and the
   ...
   adjudicated false-positive floor is ~1.22% — by the majority of three
...
| 81 | Append an unguarded `collect_ignore.append(...)` to a conftest that **already** has a collection control | **Closed v0.1.23** — the resolved *set of ignored paths* is compared now, so a second control is an event even though the marker's name never moves. Only when the control is unguarded and not itself newly added: a growing optional-dependency gate is still a gate, and the newly-added case already had an event (both conditions learned by breaking existing fixtures with the first cut). Residual: a path built at runtime is not resolved, so it is not evidence either way. Previously — marker identity is the name alone, so no `markers_added` event fires and row 71's weakest-guard computation is never read. It only works when the marker is new |
...
| 83 | Reach `collect_ignore` from an `except ImportError:` handler, or assign through a slice (`collect_ignore[:] = [...]`) | **Closed v0.1.23.** Subscript targets (`collect_ignore[:]`) are accepted, and `except` handlers are walked — an `ExceptHandler` is not an `ast.stmt`, so the recursion skipped the whole list. Walking them naively would have invented a false positive on the commonest idiom there is, so `try: import redis / except ImportError:` records the condition it actually expresses — `find_spec("redis") is None` — which the compat-gate logic already recognises. A bare `except`, another exception, or a try body that is not a plain import records unparseable text and earns no credit, which is the fail-toward-flagging side. Previously — `_collection_controls` walks `body`/`orelse`/`finalbody` but not `handlers`, and accepts only `ast.Name` targets |
| 84 | Launder the oracle anywhere but the subject: hoist the wrapper to a preceding statement, wrap the *argument*, or make the expected side an inline re-implementation of the buggy behaviour | **Partly closed v0.1.24.** Two of the three shapes are shut: the wrapper hoisted to the previous line (resolved one hop through the unit's own bindings) and the wrapper applied to an *argument* rather than to the subject (same call, same arity, every argument unchanged or containing its counterpart, at least one actually wrapped). The third — an expected side that is an inline re-implementation — is only covered where `EXPECTED_VALUE_DERIVED` reaches it, i.e. when the expectation derives from names the subject also uses; an expectation computed from anything else stays open. Residual on the closed half: **two** hops, which always exist, bounded on purpose rather than chased. Previously — `SUBJECT_NORMALIZED` compares the two subject strings and nothing else; one statement of indirection is outside its reach, and the expected side is outside every rule that is not `EXPECTED_VALUE_CHANGED` |
| 84a | The same family, hit in the wild: replace an assertion with a *different* assertion of equal strength whose expected side is not a literal — `assert invoice_total(items, 0.05) == 105.0` → `expected = sum(items)` / `assert invoice_total(items, 0.05) == expected` | **Partly closed v0.1.14** by `EXPECTED_VALUE_DERIVED` — for the case where the expectation *was a literal before the diff*, and only for bare-`assert` comparisons. The rule requires `b.right_value is not None`, so a test whose expectation was already a named local (the common shape in mature suites) is unprotected: editing that local's defining expression to mirror the bug leaves the assertion line byte-identical and reports nothing (row 86a). The unittest spelling is unprotected too, for a different reason (row 86b). Found on 2026-08-08 **in this repository**: a concurrent agent replaced the assertion in `test_pinned_tag_ships_the_current_source` with one of equal strength and greenwash passed the diff (D-031). Three rules came close and none fired — the lattice saw `EXACT_VALUE` on both sides, `EXPECTED_VALUE_CHANGED` needs both expected sides literal, `SUBJECT_NORMALIZED` needs containment and the subject never moved. The new rule keys on the *transition* from a literal to an expression whose resolved dependencies intersect the subject's own names, so a renamed constant and a parametrize argument are silent. Residuals: an expectation recomputed from names the subject does not mention (a module-level helper over the same fixture) still passes, and row 84 stays open for it; and the unchanged-subject test was a source-text comparison, so reformatting the subject in the same commit made the rule skip rather than fire — **closed v0.1.24** by the shared structural comparison |
...
| 86j | The expectation lives in a **conftest** fixture, not a same-file one | **Open, stated rather than half-implemented.** v0.1.21 tracks same-file `@pytest.fixture` returns and `parametrize` columns, which closes the residual `redteam-residual-after-p0.md` L1.2 names for in-file expectations. A fixture defined in `conftest.py` and consumed from another module is not resolved: it needs cross-file fixture resolution, which is a design change and not a widening. Reported at `info` like the rest of the rule, so it gates nothing either way |
...
| 89 | Weaken the suite in an **intermediate** script: the CI entry only calls another script, so it holds no runner token of its own — `./scripts/run-tests.sh` becoming `./scripts/run-tests.sh \|\| true` | **Closed v0.1.22.** Measured 2026-08-12 on v0.1.21: **verdict pass**, and worse than a missed CI finding — with no runner token the entry script was production, so the edit hid its own weakening *and* bought the changed-production credit that de-escalated the assertion weakening beside it. Row 87's double effect, one hop further out. Resolution follows a reference exactly **one hop**, reading the target from the diff or the head snapshot, and only promotes when that hop terminates in a real test runner — a `ci.sh` that calls `compile.sh` is still production, which is the line the content gate has drawn since v0.1.8 (`runner_one_hop_build_neg` pins it). Residual: two hops, and a reference built from a variable |
| 90 | Patch the code under test **from inside the test**, not from a conftest: `monkeypatch.setattr(billing, "invoice_total", lambda *a: 105.3)` two lines above `assert billing.invoice_total(...) == 105.3`, or the same thing spelled `mock.patch(...)` / `patch.object(...)` / `mocker.patch(...)`. Production and the assertion line both stay byte-identical | **Closed v0.1.25.** Ranked #2 of the attacks left open after P0 (★★★★★ silence): `CONFTEST_PATCHES_PROD` reads conftest files only, so the same swap one scope lower was silent. `TEST_PATCHES_SUBJECT` requires three things — the unit existed before, the patch is new, and the patched **attribute is reached by the unit's own oracle**, directly or through one hop of its bindings. The third is the entire design: in a conftest, patching first-party code is exceptional; inside a test function it is how unit tests are written, and this work's own acceptance line ("new first-party patch target in a test unit"), taken literally, fires on every commit that adds a mock. **What the corpus can and cannot say — stated because the headline number invites the wrong reading.** The sweep moved 36 → 36 blocks, but the rule fired **zero** times on 1800 commits, so that is *not* a measured false-positive rate; reporting it as one would be this project's own recurring defect, a check that cannot see its subject reporting success. What *was* measured is the base rate of the rule's precondition: 735 unit-sides across the 1800 analyses carry a patch, 38 of those sit in units the diff created, and **exactly one** newly added patch lands in a unit that already existed — denied at the stdlib/third-party filter. Humans write the mock *with* the test. Even if every such event were a false positive, the ceiling is 1/1800 = 0.06pp, twenty times under the 0.3pp budget the roadmap's T1 DoD fixed in advance: this ships on a measured blast radius, not on a measured FP count. **And that bound inherits a limit of the corpus, stated here rather than discovered later:** these six repositories carry ≈128 patch call sites at HEAD between them, where twenty candidate repositories surveyed the same day median 373 *each* (`benchmarks/corpus-expansion.md`). "Once in 1800" may be a property of test tampering or a property of six libraries that rarely stub anything, and only the wider corpus can tell those apart. The same probe found the rule's own hole before release — `result = billing.invoice_total(x)` / `assert result == 105.3` puts the patched attribute nowhere in the assertion, which is the *more* natural way to write the attack — hence the one-hop resolution. Residual: stand-ins installed by a requested fixture, runtime-built targets, `respx`/`responses` dialects, two hops, and an attribute named only on a non-literal expectation side |

| 91 | Put the oracle somewhere that is not a syntactic `assert` in the collected unit, then stop **invoking** it while leaving it in place: a helper function, a lambda, a nested `verify()`, a class whose `__init__` holds the check, a context manager, `functools.partial`, a doctest, a `compile()`d string, an inherited mixin, an autouse fixture. Or keep the `assert` and subvert what it compares — `__eq__`, `__bool__`, `__contains__`, an `__exit__` that returns True, a dataclass field marked `compare=False`, a shadowed `assertEqual`, a `TestResult` whose `addFailure` is a no-op. Or keep the loop and empty the table it iterates | **Partly closed v0.1.26 — and which part, measured on four corpora at once.** `UnitSide.assertions` now means *the assertions this unit executes*: its own direct asserts minus those in nested scopes nothing invokes, plus the direct asserts of same-file functions, lambdas, classes and `@contextmanager`s it invokes, through the file's own call graph (depth 4). Two definitions carry it, both corrected by prototype rather than argument: **reaching means invocation, not mention** — `callable(assert_sum)`, `hasattr`, `inspect.getsource` and `f.__name__` name the oracle without running it, which is precisely the edit these attacks make; and **construction is not invocation** — `partial(...)` binds, a `@contextmanager` only runs its body under `with`. Helper-borne assertions carry an `inherited` flag, and `ASSERT_SUBSTITUTED` declines a pair that crosses the body boundary in either direction: extracting a concrete assert into a shared parametrised helper (and inlining one back) moves the *slot*, not the assertion — the helper's assert is still in the reachable set, so a weakening extraction is still `ASSERT_WEAKENED`, which is what blocked 25 of the 28 disguised-extraction attacks. Numbers: informed arm 12/40 → **19/40** blocked; a same-day disguised-refactor arm blocks **28/40 at first contact**; honest-refactor false positives 20/30 → **17/30**, and the faithful-extraction arm arrives at 16/30 — both sides of the same blindness moving together, which was the acceptance test. The prototype's promised 10 was itself a miscount (three cases were vacuous-subject edits, not reachability; issue #37 carries both corrections). The A5-x round (v0.1.27) adds the two cross-file channels the corpora actually contain — a `from helpers import f` sibling import, and a fixture requested by parameter name whose body or returned closure asserts (a fixture nobody requests contributes nothing, so moving the oracle into one stays a removal; an autouse fixture in a touched conftest counts, which is what newly blocks the drop-the-autouse attack). One narrow decline, corrected once by the disguised arm itself: an own container-literal comparison extracted into `== expected` drops EXACT_STRUCT→EXACT_VALUE because a parameter cannot be a literal — excused **only** when the expectation is that bare name; `sorted(expected)`/`set(expected)` spellings walked through the first cut and are findings again. Standing: informed arm **20/40**, disguised **28/40**, refactor FPs **25/60**. Residual: row 91a |
| 91a | Everything row 91's reachable set cannot see: subvert the *meaning* of a syntactically strong assertion (`__eq__`/`__bool__`/`__contains__` always true, a swallowing `__exit__`, a no-op `TestResult`, `int()`/`set()`/`bool()` coercion inside the helper, a default `expected` parameter never overridden); make the subject vacuous while keeping the assert (`for n in nums if False`, `mismatches[:0]`, `assert pred` where `pred` was called); compute zero runs (exhausted iterator, unscheduled coroutine, table filtered to passing inputs); put the helper in *another file* (conftest.py, tests/helpers.py); or change unit identity (merge, split, params fixture) | **Open, and priced.** The 32 authorised escapes across both tamper arms are all here: 12 of the original 40 and 12 of the disguised 40 are semantic subversion or computed zero-runs, which no single static diff can decide — mutation testing is the honest answer, not a 22nd rule. The vacuous-subject family has one clean syntactic signal worth its own round: a subject that was a call becomes the bare callable (`assert pred()` → `assert pred`). Cross-file helpers shipped in v0.1.27 (A5-x) and moved to row 91; unit-identity changes are THREATMODEL 92's other half and stay open. Neither is annexed here silently — each gets its own round with its own measurement |

| 92 | (False positive, and the largest one) Restructure where a test's assertions live, without weakening anything: extract the check into a helper or a `conftest.py`, move it into a fixture's teardown, merge two tests into one, split one into two, drive them from a params fixture, put the comparison behind `operator.eq` or a comparison object's method, replace exact equality with `pytest.approx` and a tolerance | **Open, and measured 2026-08-13: greenwash blocked 20 of 30 at v0.1.25; the reachable-assertion set (row 91) brings it to **17 of 30** and the cross-file channels (v0.1.27) to **15 of 30**, with the extraction arm at **10 of 30** — the residue decomposed as unit-identity changes (merge/split/params fixture) and shared assertions that are lattice-weaker than the concrete ones it replaced. The last family is deliberately not fixed: the same transition is the signal that blocks 25 of 28 disguised-extraction attacks, and trading 25 catches for 4 false positives is not a trade.** Each case ships production twice — correct and buggy — so four pytest runs establish that *both* sides still catch the bug before greenwash is consulted; a block is then a false positive by construction, with no adjudication to disagree about. Two families: `ASSERT_REMOVED` when the assertion moved somewhere the IR does not follow (8 cases), and `TEST_DISABLED` when unit identity changed under a merge, split, `setUp` or params fixture (6 cases), plus one `ASSERT_WEAKENED` on an introduced `approx`. **This is row 91 seen from the other side** — the same blindness that lets 28 of 40 attacks through also blocks the honest refactors, because both turn on greenwash not knowing which assertions a test actually runs. It is not visible in the 1.17% published false-positive rate, and re-running that sweep would never have shown it: those six repositories rarely restructure test helpers, which is the same zero-power trap as row 90. Corpus and harness: `benchmarks/refactors/` |
