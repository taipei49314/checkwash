...
| `EXPECTATION_DEFINITION_CHANGED` | an aligned assertion whose **text is unchanged** and whose expectation resolves to something outside the assertion whose definition changed: a unit-local binding, a `parametrize` **column the expectation actually consumes**, or a same-file `@pytest.fixture`'s return/yield. Which column is the expectation is decided by consumption, not by position or by the name `expected`. Row additions and deletions are excluded — those are `TEST_DISABLED`'s event — and a cell is read through `pytest.param(...)`, so marking rows skipped is not an expectation edit. Compared structurally, so reformatting is not a change. Reported at **`info`** and never escalated: it is outside `ORACLE_RULES`, takes no repair-evidence path, and cannot gate. The severity is a measured decision, not a hedge — as a blocking rule it cost 12 extra blocks on 1800 human commits |
